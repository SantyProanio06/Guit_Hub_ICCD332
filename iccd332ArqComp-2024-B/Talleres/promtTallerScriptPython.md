...
Hola chat, oye ayúdame a resolver una parte de un taller de programación estructurada en Python que nos mandó mi profe. Me encargo de hacer la estructura inicial y la recolección de datos por consola.

Específicamente necesito que me guíes con lo siguiente:

1. **Estructura de archivos:** Dime cómo armar la carpeta `utils` y crear ahí dentro el archivo `projectile_functions.py`. También confírmame si necesito algún archivo extra (como un `__init__.py`) para que mi equipo pueda importar esto como un módulo en el `main.py`.
2. **Programación del Parser:** Dentro de `projectile_functions.py`, necesito que me ayudes a programar una sola función que importe y use `argparse` para leer parámetros desde la terminal.
3. **Argumentos a capturar:** Necesito leer la Velocidad inicial (`-V`), el Coeficiente de resistencia (`-B`), el Tiempo máximo (`-t`) y los Ángulos de lanzamiento (`-a`).
4. **Restricciones de datos:** Todos estos argumentos deben configurarse para que sean de tipo flotante (`float`). Además, en el argumento de los ángulos (`-a`), necesito que el usuario pueda ingresar varios valores a la vez (creo que se usa `nargs='+'`).
5. **Descripciones:** Agrega un parámetro `help` en cada argumento donde expliques para qué sirve y qué tipo de dato es.

Para que tengas todo el contexto exacto del problema de física y los requisitos de YASnippet en Emacs, aquí te dejo el documento completo del taller:
